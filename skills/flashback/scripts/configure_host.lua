...
local function run()
    ...
    print(string.format("[Flashback] saved host %s:%d", cfg.host_ip, cfg.port))

    local healthy, detail = health_check(cfg.host_ip, cfg.port)
    if healthy then
        print("[Flashback] /health OK")
    else
        print("[Flashback] /health not ready: " .. tostring(detail))
        print("[Flashback] ensure the Windows app is running and http_allowlist includes this IP")
    end
end
